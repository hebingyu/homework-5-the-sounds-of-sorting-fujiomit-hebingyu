...
import edu.grinnell.sortingvisualizer.sorts.Sort;

/**
 * The Control Panel houses the GUuuu''''''I for interacting with the Sounds of
 * Sorting application.
 */
public class ControlPanel extends JPanel {
    ...
    private static List<SortEvent<Integer>> generateEvents(String sort, Integer[] arr) {
        ...
        case "Merge":
            return Sort.mergeSort(arr,0, arr.length-1);
        case("Quick"):
            return Sort.quickSort(arr,0 , arr.length-1);
        default:
    ...
    public ControlPanel(NoteIndices notes, ArrayPanel panel) {
        ...
        notes.initializeAndShuffle(notes.getNotes().length);
        this.panel = panel;
        ...
        playButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                List<SortEvent<Integer>> actlist = generateEvents((String) sorts.getSelectedItem(), arr);
                for (int i = 0; i < notes.getNotes().length-1; i++) {
                    CompareEvent<Integer> compare = new CompareEvent<>(i, i + 1);
                    actlist.add(compare);
                }
                ...
                timer.schedule(new TimerTask() {
                    ...
                    public void run() {
                        ...
                        if (index < actlist.size()) {
                            SortEvent<Integer> e = actlist.get(index++);
                            e.apply(notes.getNotes());
                            for (int i = 0; i < e.getAffectedIndices().size(); i++) {
                                int sd = e.getAffectedIndices().get(i) % scale.size();
                                scale.playNote(sd, e.isEmphasized());
